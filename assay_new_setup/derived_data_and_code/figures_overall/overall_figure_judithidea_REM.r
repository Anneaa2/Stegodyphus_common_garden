...
#------------- Overall figure info
if (	fig_types == "overview"	)	{		
	if (	output_type == "pdf"	)	{ 	
		pdf(paste0("assay_new_setup/derived_data_and_code/figures_overall/",coltest_type,"overview_fig.pdf"),width=7,height=10.5) 		# a4 page is 8.3x11.7
	}else if ( 	output_type == "png"	){
		png(paste0("assay_new_setup/derived_data_and_code/figures_overall/",coltest_type,"overview_fig.png"),width=7,height=10.5,units="in",res=400) 	
	}
...
if (	fig_types == "individuel_panels"	)	{ 	
	if (	output_type == "pdf"	)	{ 	
...
if(		fig_types == "individuel_panels"	) {		mainname="Effect of temperature"; xlab_name=expression(paste("Acclimation Temperature (", degree,"C)"))	}else{ mainname="Effect of temperature\non growth"; xlab_name="Temperature"}
...
for ( popul in levels(Grow$Population) ){
	...
	#subset_dat$Treatment <- subset_dat$Treatment+jitter_move[counter]
	if ( popul == levels(Grow$Population)[1]	){		
		par( new=F)
		plot(jitter(subset_dat$Treatment+jitter_move[counter], factor=.15), subset_dat$growth, xpd=NA, 	col=popcols[counter], ylab=NA,xlab=xlab_name,		ylim=ylimsis,xlim=c(14.5,29.5),xaxt="n", las=2)#, ylim=c(44,54))
		...
		#title(main=mainname, line=main_line_out, xpd=NA)
		#if(		fig_types == "individuel_panels"	) {
		#	axis(1, at=c(15,17,19,21,23,25,27,29), labels=NA, xpd=NA)
		#}else{ 
		...
		plot(jitter(subset_dat$Treatment+jitter_move[counter], factor=.15), subset_dat$growth, 	xpd=NA,	col=popcols[counter], ylab=NA, xlab=NA, ylim=ylimsis, xlim=c(14.5,29.5),yaxt="n", xaxt="n")#, ylim=c(44,54))
	}
	#plot(jitter(subset_dat$Treatment), subset_dat$growth, 	col=popcols[counter], ylab="growth",xlab=NA,main="Effect of temperature\non survival")#, ylim=c(44,54))
...
if(		fig_types == "individuel_panels"	) {		mtext("a",side=3,line=0, at=13,cex=1.2)	}else{ }
hsdtest <- HSD.test(lm(growth ~ Treatment,data=Grow), "Treatment", group=TRUE)
hsdtest$groups
#plot(Grow$Treatment, Grow$growth,col=col_vec[9], ylab="growth factor",xlab=NA,main="Effect of temperature\non growth")#, ylim=c(44,54))
#abline(lm(Grow$growth ~ Grow$Treatment),col=col_vec[9])
...
if(		fig_types == "individuel_panels"	) {		mainname=NA	; xlab_name=expression(paste("Acclimation Temperature (", degree,"C)")) }else{ mainname="Effect of temperature\non survival" ; xlab_name="Temperature"}
ylimsis=c(min(Grow$survival,na.rm=T),	max(Grow$survival,na.rm=T) + (max(Grow$survival,na.rm=T)-min(Grow$survival,na.rm=T))*proportion_to_increase_ylim)	#c(.3,1)
for ( popul in levels(Grow$Population) ){
	counter=grep(popul,levels(Grow$Population))
	print(popul)
	plot_rows <- which(Grow$Population == popul)
	...
	if ( popul == levels(Grow$Population)[1]	) {		
		par( new=F)
		message("entering if")
		plot(jitter(subset_dat$Treatment+jitter_move[counter], factor=.15), subset_dat$survival, xpd=NA, 	col=popcols[counter], ylab=NA,	xlab=xlab_name,		ylim=ylimsis,xlim=c(14.5,29.5),xaxt="n", las=2)#, ylim=c(44,54))
		#title(main=mainname, line=main_line_out, xpd=NA)
		...
		par( new=T)
		message("entering if else")
		plot(jitter(subset_dat$Treatment+jitter_move[counter], factor=.15), subset_dat$survival, xpd=NA, 	col=popcols[counter], ylab=NA, xlab=NA, ylim=ylimsis, xlim=c(14.5,29.5),yaxt="n", xaxt="n")#, ylim=c(44,54))
	}
	#plot(jitter(subset_dat$Treatment), subset_dat$survival, 	col=popcols[counter], ylab="Survival",xlab=NA,main="Effect of temperature\non survival")#, ylim=c(44,54))
...
if(		fig_types == "individuel_panels"	) {		mtext("b",side=3,line=0, at=13,cex=1.2)	}else{ }
#plot(jitter(Grow$Treatment), Grow$survival,col=col_vec[10], ylab="Survival",xlab=NA,main="Effect of temperature\non survival")#, ylim=c(44,54))
	# if letters are wanted:
letters_are <- CLD(emmeans (surv_model_T,specs=pairwise ~ Treatment)$emmeans,Letters=c("a","b","c","d","e","f"))[,c(1,7)]
letters_are




if (	fig_types == "individuel_panels"	)	{ 	dev.off()		}
...
if(		fig_types == "individuel_panels"	) {		mainname="CTmax"; xlab_name=expression(paste("Acclimation Temperature (", degree,"C)"))	}else{ mainname="Effect of temperature\non CTmax"; xlab_name=expression(paste("Temperature (", degree,"C)"))}
...
for ( popul in levels(ToD_final$Population) ){
	...
	#plot(jitter(subset_dat$Treatment), subset_dat$ToD_mean, 	col=popcols[counter], ylab="ToD_mean",xlab=NA,main="Effect of temperature\non survival")#, ylim=c(44,54))
	...
	#print(summary(lm(subset_dat$ToD_mean ~ subset_dat$Treatment)))
...
if(		fig_types == "individuel_panels"	) {		mainname="CCR"	; xlab_name=expression(paste("Acclimation Temperature (", degree,"C)")) }else{ mainname="Effect of temperature\non CCR"; xlab_name=expression(paste("Temperature (", degree,"C)")) }
...
for ( popul in levels(CCR_final$Population) ){
	...
	#plot(jitter(subset_dat$Treatment), subset_dat$CCR_mean, 	col=popcols[counter], ylab="CCR_mean",xlab=NA,main="Effect of temperature\non survival")#, ylim=c(44,54))
...
if (	fig_types == "individuel_panels"	)	{ 	
	if (	output_type == "pdf"	)	{ 	
		pdf(paste0("assay_new_setup/derived_data_and_code/figures_overall/",coltest_type,"figure_3_Meth_Exp.pdf"),width=7,height=3) 		
	}else if ( 	output_type == "png"	){
		png(paste0("assay_new_setup/derived_data_and_code/figures_overall/",coltest_type,"figure_3_Meth_Exp.png"),width=7,height=3,units="in",res=400) 	
	}
...
#axis(1, at=c(0,seq(2000,14000,by=2000)), labels=c(0,NA,4000,NA,8000,NA,12000,NA),xpd=NA, col="white")
...
plotaa <- barplot(rev(meth_dat),        beside=T, horiz=T, col=opaque_col1(col_vec[1]), 
		...
		ylab= NA, xpd=NA,  xlim=c(0,1600), xaxt="n")
...
barplot(rev(c(overlap_genes_exprP_methP,0,0,0)),        beside=T, horiz=T, col="black", names.arg=NA,
		density=c(rep(17,3)), angle=c(rep(135,3)), xpd=NA, add=T, xlim=c(0,1600), xaxt="n")
axis(1, at=c(0,400,800,1200,1600), labels=c(0,400,800,1200,1600))
...
#arrow_x <- 	datis[1] / nrow(exp_dat) * 	meth_dat[1]		# ( N diff exp G Pop / N All genes ) * N diff METH genes
...
ggsave(plot=eul_plot,	filename=paste0("assay_new_setup/derived_data_and_code/figures_overall/",coltest_type,"_EULER_exp_meth.pdf"), 
		height=6 ,width=6 ,units="in")
ggsave(plot=eul_plot,	filename=paste0("assay_new_setup/derived_data_and_code/figures_overall/",coltest_type,"_EULER_exp_meth.png"), 
...
if (	fig_types == "individuel_panels"	)	{ 	dev.off() 		}
####################################
#------------- Metabolites ## 1 overall  plot ## From tobias LCMS
####################################
'
if (	fig_types == "individuel_panels"	)	{ 	
	if (	output_type == "pdf"	)	{ 	
		pdf(paste0("/faststorage/project/spider2/assay_study_anne/figures_overall/",coltest_type,"figure_Metabo_LCMSall.pdf"),width=7,height=6) 		
	}else if ( 	output_type == "png"	){
		png(paste0("/faststorage/project/spider2/assay_study_anne/figures_overall/",coltest_type,"figure_Metabo_LCMSall.png"),width=7,height=6,units="in",res=400) 	
	}		
	par(mfrow=c(1,1), mar=c(5.1,6,4.1,2.1))	
}

metab_file = "/faststorage/project/spider2/assay_study_anne/metabolites/data/LCMS_tobias/Significance_table_ANOVA_metabolites_LCMS.csv"
metab_dat <- read.csv(metab_file,	header=T, stringsAsFactors=T, row.names=1)
metab_dat <- metab_dat[,-ncol(metab_dat)]

# datname_vec 	popsig_vec tempsig_vec pop_temp_sig_vec intsig_vec pop_temp_int_sig_vec
#1 LCMS_allPerf         29          19                7         17                    2
#2   LCMS_CTmax         32          45               15         40                    8
#3     LCMS_CCR         26          21                7          4                    1


# grep data for all samples:
row_to_grep <- grep("CTmax|CCR",rownames(metab_dat), invert=T)
names(metab_dat)<-NULL
plotaa <- barplot(rev(as.matrix(metab_dat[row_to_grep,])),   beside=F, horiz=T, col=c(opaque_col1(col_vec[8])), xlab="Number of metabolites",main= "LCMS - ALL", names.arg=NULL, xpd=NA)
text(	x=-max(metab_dat[row_to_grep,]) * .09,
		y = plotaa, 
		labels=colnames(custom_dat), xpd=NA,
		srt=45, adj=c(.9,1.5))
...
text(	x=-max(metab_dat_sub[row_to_grep,]) * .09,
		...
		labels=colnames(custom_dat), xpd=NA,
		srt=45, adj=c(.9,1.5))
if(		fig_types == "individuel_panels"	) {		mtext("a",side=3,line=0.5, at=-max(metab_dat_sub[row_to_grep,]) * .05,cex=1.2)	}else{ }
...
labels_are=seq(0,45,10)	; axis(1, at=labels_are, labels=T, tcl=-.5)
labels_are=seq(0,45,5)	; labels_are[seq(1,NROW(labels_are),2)] <- NA	;	axis(1, at=labels_are, labels=NA, tcl=-.25)
if(		fig_types == "individuel_panels"	) {		mtext("b",side=3,line=0.5, at=-max(metab_dat_sub[row_to_grep,]) * .05,cex=1.2)	}else{ }
...
text(	x=-max(colSums(metab_dat_CTmax)) * .06,
		...
		labels=colnames(custom_dat), xpd=NA,
		srt=45, adj=c(.9,1.5))
if(		fig_types == "individuel_panels"	) {		mtext("c",side=3,line=0.5, at=-max(colSums(metab_dat_CTmax)) * .04,cex=1.2)	}else{ }
...
if(		fig_types == "individuel_panels"	) {		mtext("d",side=3,line=0.5, at=-max(colSums(metab_dat_CCR)) * .05,cex=1.2)	}else{ }

if (	fig_types == "individuel_panels"	)	{ dev.off() 		}
if (	fig_types == "overview"	)	{	 dev.off() 		}
print(metab_dat)
