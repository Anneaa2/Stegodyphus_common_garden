...
sort -s -k 13,13 $directory/$file | awk '
		 ...
		 {m=m+$5;u=u+$6;count++}
		 END{print gene,pos,m+u,m,count;m=0;u=0;pos=0;count=0}' > $output_directory/${file/.$extension/_inputDSS.$extension".count"}		

# m = methylated , u = unmethylated, pos= position of starting cytosine

	# Cut the count column awai for DSS
cut -d$'\t' -f 1,2,3,4 $output_directory/${file/.$extension/_inputDSS.$extension".count"}		> $output_directory/${file/.$extension/_inputDSS.$extension"f"}		

	# Add header for files
sed -i '1s/^/chr\tpos\tN\tX\n/' $output_directory/${file/.$extension/_inputDSS.$extension"f"}		
