...
do
	...
	sbatch assay_new_setup/derived_data_and_code/methylation/coverage_filter_awk.sh /faststorage/project/spider2/Methylation_data/Lin_20180608_bismark_v2_assay/SMP/$file assay_new_setup/derived_data_and_code/methylation//smps/${file/.cov/_d10D32.cov} 10
done
...
do
	qx --no-scratch -m 50G "bedtools intersect -wo -a assay_new_setup/derived_data_and_code/methylation/smps/$file -b /faststorage/project/spider2/Methylation_data/AAL_20200207_fishing_regions_SOV/gene_region/gene_pos_region.txt > assay_new_setup/derived_data_and_code/methylation/smps/gene_region/${file/.cov/_gene.cov}"
done
...
mv cal_weighted_mlvl.sh*out outfiles/calc_wmlvl_outs/
...
do 
	...
	#do
	#	echo $directory/$file
	#	sbatch make_input_for_dss.sh $directory $file
	#done
...
# for directory in `ls -d assay_new_setup/derived_data_and_code/methylation/smps/*region`		# if I want to look in all regions extracted
#do 
	for file in `ls $directory | grep ".d10D32_" | grep ".cov$"`
	do
		grep "CpG" $directory/${file} > $directory//${file/.cov/.cpg}
...
for file in `ls assay_new_setup/derived_data_and_code/methylation/smps/gene_region/ | grep ".d10D32_" | grep ".cov$"`
	do
		sbatch --account=spider2 --wrap="grep "CHH" assay_new_setup/derived_data_and_code/methylation/smps/gene_region/${file} > assay_new_setup/derived_data_and_code/methylation/smps/gene_region/${file/.cov/.chh}"
		sbatch --account=spider2 --wrap="grep "CHG" assay_new_setup/derived_data_and_code/methylation/smps/gene_region/${file} > assay_new_setup/derived_data_and_code/methylation/smps/gene_region/${file/.cov/.chg}"
done
...
for file in `ls assay_new_setup/derived_data_and_code/methylation/smps/gene_region/ | grep ".d10D32_" | grep ".chh$\|.chg$"`
do
	echo assay_new_setup/derived_data_and_code/methylation/smps/gene_region/$file
	extension=${file##*.}
	echo $extension
	echo ${file/.$extension/.$extension.sum}		
	sbatch assay_new_setup/derived_data_and_code/methylation/calculates_sums.sh assay_new_setup/derived_data_and_code/methylation/smps/gene_region/ $file	
done
	# Outfile loooks like this: Gene	methylated_reads		unmethylated_reads		count_of_Cs_in_region
	# m = methylated , u = unmethylated
...
for file in `ls assay_new_setup/derived_data_and_code/methylation/smps/gene_region/ | grep ".d10D32_" | grep ".chh.sum$\|.chg.sum$" |head -n2`
do
	echo assay_new_setup/derived_data_and_code/methylation/smps/gene_region/$file
	extension=${file#*.}
	echo $extension
	sbatch assay_new_setup/derived_data_and_code/methylation/cal_weighted_mlvl.sh assay_new_setup/derived_data_and_code/methylation/smps/gene_region/ $file	
done
...
do
	echo assay_new_setup/derived_data_and_code/methylation/smps/gene_region/$file
	sbatch assay_new_setup/derived_data_and_code/methylation/make_input_for_dss.sh assay_new_setup/derived_data_and_code/methylation/smps/gene_region/ $file	
done
...
for file in `ls assay_new_setup/derived_data_and_code/methylation/smps/gene_region/ | grep ".d10D32_" | grep "_inputDSS.chgf$\|_inputDSS.chhf$" |head -n2`
do
	echo assay_new_setup/derived_data_and_code/methylation/smps/gene_region/$file $file
	sbatch assay_new_setup/derived_data_and_code/methylation/dss_run.sh assay_new_setup/derived_data_and_code/methylation/smps/gene_region/ $file	
done
