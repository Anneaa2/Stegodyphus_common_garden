...
file_is=argsis[2]		#B15_d5D32_gene_inputDSS.cpgf
				...
				#directory_is="/faststorage/project/spider2/assay_study_anne/methylation/smps/second_analysis_DSS//"
				#file_is="B15_dssINPUT2.cpg"
...
print(strsplit(file_is,split="_"))
patternis=sapply(strsplit(file_is,split="_"),"[[",3)
print(patternis)
...
for (each in files_are){
	message(each)
	sample_name=sapply(strsplit(each,"/|_"),"[[",15) #Gets the sample name (uniq) --> Adjust to your needs
	message(sample_name)
	...
	colnames(varlist[[nam]]) <- c("chr","pos","N","X")
...
write.table(effect_of_tab, file=paste0(directory_is,"/DSS_result_counteffects_",outname,".txt"),row.names=F,col.names=T,quote=F,sep="\t")



# make a merged results table:

merged_table <- join_all(list(test_1,test_2), by =c("chr","pos") , type = 'left')
...
write.table(merged_table, file=paste0(directory_is,"/DSS_result_all_",outname,".txt"),row.names=F,col.names=T,quote=F,sep="\t")
